...
import com.Lalitdk.project.uber.uberApp.repositories.RatingRepository;
  ...
  @Override
    public DriverDto rateDriver(Ride ride ,Integer rating) {
        Driver driver = ride.getDriver();
         Rating rideObjRide = ratingRepository.findByRide(ride)
        .orElseThrow(() -> new ResourceNotFoundException("Rating not found for ride with ID: " + ride.getId()));

        if (rideObjRide.getDriverRating()!=null){
            throw new RuntimeConflictException("Driver has been rated already, cannot rate again");
        }
        rideObjRide.setDriverRating(rating);

        ratingRepository.save(rideObjRide);
    ...
    public RiderDto rateRider(Ride ride, Integer rating) {
        ...
        if (ratingObj.getRiderRating()!=null){
            throw new RuntimeConflictException("Rider has been rated already, cannot rate again");
        }
